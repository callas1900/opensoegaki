...
- [x] #5 While the crop tool is active on an image there is always a visible region: Cancel (on-canvas control or Esc) resets it to the full image without changing the document; apply and undo re-initialize it on the resulting image; switching tools or pasting exits/reinitializes cleanly
<!-- AC:END -->
