...
- [x] #5 The lens border strokes at 1.5x strokeWidth and the source ring at 0.9x, in the unchanged two-pass white-outline style, with no seam or gap where the connector meets either rim (device-checked on a phone-sized and a desktop-sized capture).
...
- [x] #8 Device-verified on a real iPhone: at minimum source size the source can still be dragged with a finger without hitting the grip, and the grip itself is comfortably grabbable.
<!-- AC:END -->
...
2026-08-17: user ran the remaining device pass and checked AC #5 and #8; task moved to Done. The 'REMAINING for Done' line above is historical.
<!-- SECTION:NOTES:END -->
